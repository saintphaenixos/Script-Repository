...
echo "#Version 2.2"
echo ""
echo "#This is a sourceable library of functions for changing the text or background of text colors to one of the 256 colors using ANSI/VT100 Control sequences."
...
for ((i=0;i<256;i++)); do
  ...
  echo "${shades[$i]}() {"
  echo "    if (( \$# == 0 )) ; then"
  echo "        xargs -r -I{} echo -e \"\e[38;5;${colors[$i]}m\"{}\"\e[0m\" < /dev/stdin"
  echo "    else"
  echo "        echo -e \"\e[38;5;${colors[$i]}m\${@}\e[0m\""
  echo "    fi"
  echo "}"
...
for ((i=0;i<256;i++)); do
  ...
  echo "        xargs -r -I{} echo -e \"\e[38;5;${colors[$i]}m{}\e[0m\" < /dev/stdin"
  echo "    else"
  echo "        echo -e \"\e[48;5;${colors[$i]}m\${@}\e[0m\""
  echo "    fi"
  echo "}"
