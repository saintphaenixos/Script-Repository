...
udevadm | pee "udevadm info --query=all --name=sd"{a..z} "udevadm info --query=all --name=sd"{a..z}{a..z} | grep -E \(S=usb\|\ sd\) | tr -d 'N: ' | tr -d 'E: ID_BUS=' | grep -B1 usb | grep sd | time parallel -j+0 --progress 'gunzip </home/kduback/Downloads/SCCMstick.img.gz >/dev/{} && fatresize -q -s 15G /dev/{}1 && dosfsck -p -t -w -v'
